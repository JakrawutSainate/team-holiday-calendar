...
export default function SettingsPage() {
  ...
  // Observe scroll to highlight tabs
  useEffect(() => {
    const handleScroll = () => {
    ...
    <div className="flex-grow flex flex-col min-h-screen">
      ...
      <main className="flex-1 p-12 overflow-y-auto custom-scrollbar">
        <div className="max-w-[1280px] mx-auto">
          <div className="mb-12">
            <h2 className="text-4xl font-bold tracking-tight text-primary mb-2">Settings</h2>
          ...
          <div className="grid grid-cols-12 gap-8">
            {/* Settings Navigation (Local Tabs) */}
            ...
            <div className="col-span-9 flex flex-col gap-12">
              {/* Profile Settings */}
              <div ref={profileRef} className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-8 scroll-mt-24">
                <div className="flex items-center justify-between mb-8">
                  <div>
                    <h3 className="text-lg font-bold text-primary">Profile Settings</h3>
                    <p className="text-xs text-secondary">Information that identifies you across the HQ.</p>
                  </div>
                  <button className="px-4 py-2 border border-outline-variant rounded-lg font-label-caps text-xs font-bold hover:bg-surface-container transition-colors cursor-pointer">
                    Edit
                  </button>
                </div>
                <div className="space-y-6">
                  <div className="flex items-center gap-4 p-4 bg-surface border border-outline-variant rounded-xl">
                    ...
                    <div>
                      <h4 className="font-label-caps text-xs text-primary font-bold">Avatar</h4>
                      <p className="text-xs text-secondary">Recommended: 200x200px</p>
                    </div>
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="font-label-caps text-[10px] font-bold text-secondary block mb-1">Full Name</label>
                      <input
                        type="text"
                        defaultValue="Alex Rivera"
                        className="w-full bg-white border border-outline-variant rounded-lg px-4 py-3 text-sm focus:border-primary focus:ring-0 transition-all outline-none"
                      />
                    </div>
                    <div>
                      <label className="font-label-caps text-[10px] font-bold text-secondary block mb-1">Email Address</label>
                      <input
                        type="email"
              ...
              <div ref={notificationsRef} className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-8 scroll-mt-24">
                <div className="mb-6">
                  <h3 className="text-lg font-bold text-primary">Notification Preferences</h3>
                  <p className="text-xs text-secondary">Choose how and when you receive updates.</p>
                </div>
                <div className="divide-y divide-outline-variant">
                  <div className="py-4 flex items-center justify-between">
                    <div>
                      <h4 className="text-sm font-bold text-primary">Email Notifications</h4>
                      <p className="text-xs text-secondary">Daily digest of leave requests and balance updates.</p>
                    </div>
                    <button
                  ...
                  <div className="py-4 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-secondary">Instant alerts for team approvals or denials.</p>
                    </div>
                    <button
              ...
              {/* Workspace Settings */}
              <div ref={workspaceRef} className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-8 scroll-mt-24">
                <div className="mb-8">
                  <h3 className="text-lg font-bold text-primary">Workspace Settings</h3>
                  <p className="text-xs text-secondary">Global rules for your team's holiday planning.</p>
                </div>
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className="flex-1 px-4 py-3 bg-white border border-outline-variant rounded-lg flex items-center justify-between">
                          <span className="text-xs text-secondary">Base Rate</span>
                          <span className="text-sm font-bold text-primary">{earnRate}</span>
                        </div>
                        <select
                          value={earnRate}
                          onChange={(e) => setEarnRate(e.target.value)}
                          className="p-3 border border-outline-variant rounded-lg bg-white text-xs hover:bg-surface-container transition-colors cursor-pointer outline-none font-bold"
                        >
              ...
              {/* Appearance */}
              <div ref={appearanceRef} className="bg-surface-container-lowest border border-outline-variant rounded-2xl p-8 mb-12 scroll-mt-24">
                <div className="mb-6">
                  <h3 className="text-lg font-bold text-primary">Appearance</h3>
                  <p className="text-xs text-secondary">Customize the interface visual tone.</p>
                </div>
                <div className="grid grid-cols-3 gap-4">
                  <div className="cursor-pointer group">
                    <div className="aspect-video bg-white border-2 border-primary rounded-xl p-1 mb-2 transition-all">
                      <div className="w-full h-full bg-surface-container flex flex-col gap-1 p-2">
                    ...
                    <span className="font-label-caps text-[10px] text-primary flex items-center gap-1 font-bold">
                  ...
                  <div className="opacity-40 cursor-not-allowed">
                    ...
                    <span className="font-label-caps text-[10px] text-secondary font-bold">Mono Dark (Coming Soon)</span>
                  </div>
                  ...
                  <div className="opacity-40 cursor-not-allowed">
                    ...
                    <span className="font-label-caps text-[10px] text-secondary font-bold">System Default</span>
                  </div>
                </div>
              </div>
              ...
              <div className="flex items-center justify-end gap-4 pb-12">
                ...
                <button className="bg-primary text-on-primary px-8 py-3 rounded-xl font-label-caps text-xs font-bold scale-95 active:scale-90 transition-transform shadow-lg shadow-primary/10 cursor-pointer tracking-wider">
                  Save Preferences
                </button>
              </div>
            </div>
